...
var y{i in MFRS} := resources[i,member(5, VACCINES)], binary;  # whether mfr i should produce the hexavalent vaccine
...
minimize WeightedObj:
    ...
    - mu2*quant[member(5, VACCINES),i,t])
    + mu3 * z;
...
subject to HexProductionCapacity1{i in MFRS, t in HORIZON}:
    quant[member(5, VACCINES),i,t] <= hexCapacity[i] * y[i]; 
...
subject to HexProductionCapacity2{i in MFRS, t in HORIZON}:
    price[member(5, VACCINES),i,t] <= hexCapacity[i] * y[i];

# Production capacity limit on existing vaccines
subject to ProductionCapacity{i in MFRS, t in HORIZON}:
    sum{v in VACCINES: v <> member(5, VACCINES)}(quant[v,i,t]) <= capacity[i]; 

# Ensure manufacturers make at least minimum profit P
subject to ProfitThreshold{i in MFRS}:
    sum{t in HORIZON, v in VACCINES}(quant[v,i,t]*price[v,i,t]) >= T*profit[i]*sum{t in HORIZON, v in VACCINES}(x[v,i,t]) + hexCost[i]*y[i];
...
subject to OPVLimit{t in HORIZON}:
    sum{i in MFRS}(quant[member(3, VACCINES),i,t]) <= (demand[member(5, ANTIGENS),t]); 
