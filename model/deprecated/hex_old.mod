...
set ANTIGENS ordered;   # antigens: 1 DTP 2 HepB 3 Hib 4 IPV 5 OPV
...
# Ensure manufacturers make at least minimum profit P (and make up for hex cost)
...
subject to NonzeroQuantity{i in MFRS, v in VACCINES, t in HORIZON: v <> member(6, VACCINES)}:
    upperDemand[v]*x[v,i,t]*resources[i,v] >= quant[v,i,t];

subject to NonzeroQuantityHex{i in MFRS, t in HORIZON}:
    upperDemand[member(6, VACCINES)]*x[member(6, VACCINES),i,t] >= quant[member(6, VACCINES),i,t]; # for hexavalent

# Nonzero price forces binary 1 (not for hex because we want mfr to be able to start produce)
subject to NonzeroPrice{i in MFRS, v in VACCINES, t in HORIZON: v <> member(6, VACCINES)}:
    upperPrice[v]*x[v,i,t]*resources[i,v] >= price[v,i,t]; 
    
# Nonzero price forces binary 1 (not for hex because we want mfr to be able to start produce)
subject to NonzeroPriceHex{i in MFRS, t in HORIZON}:
    upperPrice[member(6, VACCINES)]*x[member(6, VACCINES),i,t] >= price[member(6, VACCINES),i,t]; # for hexavalent
...
subject to OPVLimit{t in HORIZON}:
    sum{i in MFRS}(quant[member(4, VACCINES),i,t]) <= (2/3)*(demand[member(5, ANTIGENS),t]); 

# Certain percentage of antigen demand must be met by Hexavalent vaccine
